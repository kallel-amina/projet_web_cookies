...
export type TiramisuProduct = Product;
...
export class TiramisuComponent {
  ...
  filteredProducts: TiramisuProduct[] = [];
...
addToCart(product: TiramisuProduct) {
  ...
  private extractPageProducts(products: Product[]): TiramisuProduct[] {
    return products.filter((product) => {
      const normalizedCategory = (product.category || '').toLowerCase();
      const pageCategory = normalizedCategory.split(':')[0].trim();

      if (pageCategory) {
        return pageCategory === 'tiramisu';
      }

      const searchableText = `${product.name} ${product.description}`.toLowerCase();
      return searchableText.includes('tiramisu');
    });
  }

  getDietaryTag(product: TiramisuProduct): string | null {
    const normalizedCategory = (product.category || '').toLowerCase();
