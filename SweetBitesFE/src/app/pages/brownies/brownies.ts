...
addToCart(product: brownie) {
  ...
  private extractPageProducts(products: Product[]): brownie[] {
    return products.filter((product) => {
      const normalizedCategory = (product.category || '').toLowerCase();
      const pageCategory = normalizedCategory.split(':')[0].trim();

      if (pageCategory) {
        return pageCategory === 'brownies';
      }

      const searchableText = `${product.name} ${product.description}`.toLowerCase();
      return searchableText.includes('brownie');
    });
  }

  getDietaryTag(product: brownie): string | null {
    const normalizedCategory = (product.category || '').toLowerCase();
